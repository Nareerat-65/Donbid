...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    if (product.status === 'live' && new Date(product.end_time) <= now) {
      ...
      const [bids] = await db.query('SELECT user_id, bid_price FROM bids WHERE product_id = ? ORDER BY bid_price DESC, created_at DESC LIMIT 1', [id]);
      if (bids.length > 0) {
        await db.query('INSERT INTO auction_results (product_id, winner_user_id, final_price, closed_at) VALUES (?, ?, ?, ?)',
